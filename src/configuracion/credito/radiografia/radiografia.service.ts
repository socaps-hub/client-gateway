...
export class RadiografiaService {
  ...
  getAllControlCargaRadiografias() {
    return this.client.send(
      radiografiasPatterns.GET_ALL_CONTROL_CARGA_RADIOS,
      {},
    );
  }

  crearCargaMasivaRadiografiaCredito(key: string, cooperativaCodigo: string) {
    return this.client.emit(radiografiasPatterns.UPLOAD_RA_CREDITO, {
      key,
      cooperativaCodigo,
    });
  }

  crearCargaMasivaRadiografia(
    key: string,
    cooperativaCodigo: string,
    area: RadioAreaEnum,
    user: Usuario,
  ) {
    return this.client.emit(radiografiasPatterns.UPLOAD_RADIOGRAFIA, {
      key,
      cooperativaCodigo,
      area,
      user,
